...
def load_sample_ids_for_sidebar(_engine):
    try:
        with _engine.connect() as conn:
            dfp = pd.read_sql(text(f"SELECT profile_id FROM {PG_SCHEMA}.profiles ORDER BY random() LIMIT 100;"), conn)
            dfu = pd.read_sql(text(f"SELECT user_id    FROM {PG_SCHEMA}.users    ORDER BY random() LIMIT 100;"), conn)
            dfc = pd.read_sql(text(f"SELECT content_id FROM {PG_SCHEMA}.content  ORDER BY random() LIMIT 100;"), conn)
        return {
            "profiles": [str(x) for x in dfp["profile_id"].tolist()],
            "users":    [str(x) for x in dfu["user_id"].tolist()],
            "contents": [str(x) for x in dfc["content_id"].tolist()],
        }
    except Exception:
        return {"profiles": [], "users": [], "contents": []}
...
with st.sidebar:
    ...
    role = st.selectbox("User role", ["viewer", "support", "content_ops", "licensing", "all"], index=0)

    # Auto-populate IDs once if empty
    try:
        if not st.session_state.get("_ids_autofilled", False):
            _sb_eng = get_pg_engine(pg_uri)
            with _sb_eng.connect() as conn:
                pid = pd.read_sql(text(f"SELECT profile_id FROM {PG_SCHEMA}.profiles ORDER BY random() LIMIT 1;"), conn).iloc[0,0]
                uid = pd.read_sql(text(f"SELECT user_id    FROM {PG_SCHEMA}.users    ORDER BY random() LIMIT 1;"), conn).iloc[0,0]
                cid = pd.read_sql(text(f"SELECT content_id FROM {PG_SCHEMA}.content  ORDER BY random() LIMIT 1;"), conn).iloc[0,0]
            st.session_state.setdefault("profile_id", str(pid))
            st.session_state.setdefault("user_id",    str(uid))
            st.session_state.setdefault("content_id", str(cid))
            st.session_state["_ids_autofilled"] = True
    except Exception:
        pass

    # Inputs bound to session_state so the helper can populate them
    profile_id = st.text_input("profile_id (UUID)", value=st.session_state.get("profile_id", ""), key="profile_id")
    user_id    = st.text_input("user_id (UUID)",    value=st.session_state.get("user_id", ""),    key="user_id")
    content_id = st.text_input("content_id (UUID)", value=st.session_state.get("content_id", ""), key="content_id")
...
try:
    ...
    with st.expander("Run Postgres query", expanded=True):
        ...
        def filter_queries_by_role(qdict: dict, role: str) -> dict:
            # If user selects 'all', do not filter anything
            if (role or "").lower() == "all":
                return qdict
        ...
        if sel in pg_q:
            ...
            if run:
                ...
                # Guard: if any required param is empty, skip running to avoid uuid cast errors
                missing = [k for k in wanted if isinstance(PARAMS_CTX.get(k), str) and not PARAMS_CTX.get(k)]
                if missing:
